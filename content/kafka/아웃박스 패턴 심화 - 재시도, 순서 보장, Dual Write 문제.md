...
### CDC의 한계 — 메시지 형태가 테이블 스키마에 종속됨

CDC가 좋다고 해서 **아웃박스 테이블 없이 업무 테이블(`reservations` 등)을 곧바로 CDC로 tail**하면 새로운 문제가 생긴다. CDC가 발행하는 메시지는 결국 그 테이블의 row 그 자체이기 때문에, 메시지 형태가 DB 스키마에 종속되고 앱이 원하는 형태로 값을 추가/가공할 수 없다.

예를 들어 `reservations(id, user_id, room_id, status, price_cents, updated_at)` 테이블을 그대로 CDC로 tail하면, 상태가 `CONFIRMED`로 바뀔 때 이런 메시지가 나간다.

```json
{
  "after": {
    "id": 1024, "user_id": 55, "room_id": 12,
    "status": "CONFIRMED", "price_cents": 150000,
...
컨슈머(예: 웹소켓 서버)가 "홍길동님의 강남점 3층 A룸 예약이 확정되었습니다"를 바로 push하려면, 이 메시지만으론 부족하다.

- `user_id: 55` → 이름/이메일을 모름 → user 서비스에 `GET /users/55` 추가 호출 필요
- `room_id: 12` → 지점/층/호실을 모름 → room 서비스에 `GET /rooms/12` 추가 호출 필요
- `price_cents`를 "150,000원"으로 포맷하거나 `isFirstReservation` 같은 파생 필드를 넣는 것도 불가능 — DB 컬럼에 애초에 없는 값이니까.

반면 앱이 같은 트랜잭션에서 직접 조인/가공한 값으로 아웃박스 테이블에 페이로드를 써두면(`{"customerName": "홍길동", "roomName": "강남점 3층 A룸", "priceFormatted": "150,000원", ...}`), CDC는 그 완성된 JSON을 그대로 배달하는 역할만 하게 되어 메시지 형태가 DB 스키마가 아니라 **앱이 정한 도메인 이벤트 계약**을 따르게 된다.

그래서 실무에서는 "업무 테이블 직접 CDC"보다 "**앱이 아웃박스 테이블에 도메인 이벤트를 명시적으로 쓰고, 그 아웃박스 테이블만 CDC로 tail**"하는 하이브리드 방식(우아한형제들 기술블로그 사례 등)을 쓴다 — CDC의 안정성(binlog 기반 유실 없는 발행)과 앱이 원하는 메시지 형태를 동시에 가져가는 절충안이다.

CDC 릴레이의 컴포넌트별 흐름(OUTBOX table → Transaction log → log miner → Message Broker)을 그림으로 뜯어본 노트: [[Transaction Log Tailing 패턴 - Outbox를 CDC로 발행하는 흐름]].
