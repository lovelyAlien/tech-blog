...
**폴링이 신호 유실을 "보완"하는 원리**: 폴링은 유실된 신호를 되살리거나 재전송하는 게 아니다. 대신, 신호가 유실됐다면 당연히 나타날 수밖에 없는 증상 — 즉 **"그 Task의 완료 플래그가 안 찍혀 있다"** — 을 감지해서, 배치가 그 상태를 보고 섣불리 진행하지 못하게 막는 것이다. 신호 자체가 아니라 신호가 만들어내야 할 "결과"를 검증하기 때문에, 신호가 어떤 이유로 유실되든 상관없이 안전하게 걸러진다. (Task마다 완료 플래그를 남기는 타이밍이 다르다는 점은 바로 위 "2차 시도" 섹션에서 이미 다뤘다 — 여기서는 그중 신호 자체를 아예 못 받은 케이스를 본다.)
