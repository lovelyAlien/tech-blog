...
**Case A: 폴링 없이 신호만 믿었다면 (가정)**
```
배치: Pause 발행 → "보냈으니 다 멈췄겠지" 하고 바로 DB 쓰기 시작
Task 2: 마침 재시작 중이라 신호를 못 받음 → 계속 메시지 소비/쓰기 중
→ 배치 write와 Task 2의 write가 겹침 → 데이터 경합 발생
```
문제의 핵심: "신호를 보냈다"는 행위 자체를 "다들 멈췄다"의 증거로 착각하고 확인 없이 다음 단계로 넘어간 것. Pub/Sub은 fire-and-forget이라 "보냈다"가 "도착했다"를 보장하지 않는데 이를 무시했다.

**Case B: 폴링으로 재확인 (실제 채택안)**
```
배치: Pause 발행 → 바로 쓰기 시작하지 않고 Redis의 완료 플래그를 폴링
Task 2: 신호 유실 → 계속 소비 중 → 완료 플래그가 안 찍힘
배치: "Task2=미확인" 계속 확인 → 대기 → 타임아웃까지도 미확인 → 배치 스킵
→ 데이터 경합 자체가 발생하지 않음 (대신 그 주기만큼 갱신 지연)
```

| | Case A (폴링 없음) | Case B (폴링 추가) |
|---|---|---|
| 신뢰 대상 | "신호를 보냈다"는 사실 | "완료 플래그"라는 결과 |
| Task가 신호를 놓치면 | 감지 못 하고 배치 강행 → 경합 발생 | 폴링으로 감지 → 배치 스킵 → 경합 없음 |
| 대신 치르는 비용 | 없음 (그래서 위험) | 확인 안 되면 그 주기만큼 갱신 지연 |

**폴링이 신호 유실을 "보완"하는 원리**: 폴링은 유실된 신호를 되살리거나 재전송하는 게 아니다. 대신, 신호가 유실됐다면 당연히 나타날 수밖에 없는 증상 — 즉 **"그 Task의 완료 플래그가 안 찍혀 있다"** — 을 감지해서, 배치가 그 상태를 보고 섣불리 진행하지 못하게 막는 것이다. 신호 자체가 아니라 신호가 만들어내야 할 "결과"를 검증하기 때문에, 신호가 어떤 이유로 유실되든 상관없이 안전하게 걸러진다.

**Task 상태에 따라 완료 플래그를 남기는 타이밍이 다르다**
- 유휴 상태(아무 메시지도 처리 안 하던 중)였던 Task → 신호를 받자마자 즉시 완료 플래그 갱신
- 메시지 처리 중이던 Task → 신호를 받아도 그 메시지는 끝까지 마저 처리한 뒤에야 완료 플래그 갱신

즉 "작업 중이었는지 아닌지"에 따라 플래그를 남기는 시점만 다를 뿐, 결국 둘 다 "지금은 안전하게 멈췄다"가 확정된 순간에만 플래그를 남긴다는 원칙은 같다.
